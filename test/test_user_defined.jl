@testset "Reproduces Cylindrical via CurvilinearCoords" begin
    h1(q) = 1
    h2(q) = q[1]
    h3(q) = 1
    ...
    @test custom_laplacian ≈ builtin_laplacian atol=TOL
end

@testset "Reproduces Spherical via CurvilinearCoords" begin
    h1(q) = 1
    h2(q) = q[1]
    h3(q) = q[1] * sin(q[2])
    radius_squared_field(q) = q[1]^2
...
@testset "Parabolic cylindrical (novel system)" begin
    h1(q) = sqrt(q[1]^2 + q[2]^2)
    h2(q) = sqrt(q[1]^2 + q[2]^2)
    h3(q) = 1
    parabolic_parameterization(q) = SVector((q[1]^2 - q[2]^2) / 2, q[1] * q[2], q[3])
